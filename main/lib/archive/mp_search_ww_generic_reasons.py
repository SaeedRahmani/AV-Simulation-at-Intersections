...
class MotionPrimitiveSearch:
    def __init__(self, scenario: Scenario, car_dimensions: CarDimensions, mps: Dict[str, MotionPrimitive],
                 ...
                 wh_dist: float = 0.25, wh_theta: float = 2.7, wh_steering: float = 15.0, wh_obstacle: float = 0.0, wh_center: float = 0.0, wh_policymaker_rightlane_reason: float = 0.25, wh_cyclist_comfort_reason: float = 0.25, wh_driver_patience_reason: float = 0.25,
        ...
        self._wh_policymaker_rightlane_reason = wh_policymaker_rightlane_reason # weight for the cost related to distance from the centerline in heuristic function
        self._wh_cyclist_comfort_reason = wh_cyclist_comfort_reason # weight for the cost related to distance from the moving obstacles in heuristic function
        self._wh_driver_patience_reason = wh_driver_patience_reason # weight for the cost related to time elapsed the driver follow the cyclist in heuristic function
    ...
    def distance_to_goal(self, node: Tuple[float, float, float]) -> float:
        ...
        # Costs calculation
        distance_xy = self.normalize_distance_to_goal(x, y, goal_x, goal_y)
        ...
        if self._moving_obstacles_state is not None:
            ...
            self._moving_obstacles_state[1] += self._moving_obstacles_state[2] * self._mps['straight'].n_seconds
            # Calculate the distance between the current node and the moving obstacles
            distance_to_moving_obstacles = np.linalg.norm(
                [x - self._moving_obstacles_state[0], y - self._moving_obstacles_state[1]])

            # Reasons Calculation
            # Reasons for policymaker
            distance_centerline = self.compute_centerline_deviation_cost(x)
            # Reasons for cyclist
            comfort_distance_cyclist = self.compute_bicycle_distance_cost(distance_to_moving_obstacles)
            comfort_time_cyclist = self.compute_bicycle_time_cost(distance_to_moving_obstacles)
            total_cyclist_comfort = comfort_distance_cyclist * comfort_time_cyclist
            # Reasons for driver
            driver_patience = self.compute_driver_patience(distance_to_moving_obstacles)

            # Weights for the human agents
            weight_agents = [0.33, 0.34, 0.33] # [cyclist, policymaker, driver]
            # Define a global scaling factor
            GLOBAL_SCALE = 200.0
            # Define the original relative weights
            base_weight_reasons = [[1.0], [1.0], [0.5, 0.5]]

            # Apply the scaling factor to all weights
            weight_reasons = [
                [GLOBAL_SCALE * value for value in sublist] 
                for sublist in base_weight_reasons
            ]

            # Heuristic cost calculation with proximity penalty
            heuristic_cost = (
                    weight_agents[0] * weight_reasons[0][0] * total_cyclist_comfort +
                    weight_agents[1] * weight_reasons[1][0] * distance_centerline +
                    weight_agents[2] * (weight_reasons[2][0] * distance_xy + weight_reasons[2][1] * driver_patience)
            )
        else:
            # Heuristic cost calculation without moving obstacles
            heuristic_cost = (
                    self._wh_dist * distance_xy +
                    self._wh_obstacle * obstacle_avoidance_cost +
                    self._wh_center * distance_from_center
            )
    ...
    def compute_bicycle_time_cost(self, distance_to_moving_obstacles):
        ...
        # Track time spent too close to cyclist
        if not hasattr(self, '_close_proximity_time'):
        ...
        if distance_to_moving_obstacles < SAFETY_DISTANCE:
            self._close_proximity_time += self._mps['straight'].n_seconds  # Add 0.3 seconds for each node call
        else:
